...
class Template_
{
	...
	function __construct($basepath=""){
		$this->compile_dir = ($basepath==""?$_SERVER["DOCUMENT_ROOT"]:$basepath)."/_compile";
		$this->template_dir = ($basepath==""?$_SERVER["DOCUMENT_ROOT"]:$basepath)."/_template";
		$this->cache_dir = ($basepath==""?$_SERVER["DOCUMENT_ROOT"]:$basepath)."/_cache";
	}
	...
	function print_($fid, $scope='', $sub=0)
	{
		...
		if (!isset($this->tpl_[$fid])) {
			// 2007-12-06 �섏젙
			// define �뺤쓽瑜��섏� �딆븯�ㅻ㈃ 湲곕낯�쇰줈 鍮��뚯씪���명똿
			$this->tpl_[$fid] = array('txt',"blank.html");
