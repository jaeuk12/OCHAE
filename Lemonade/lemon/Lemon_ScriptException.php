...
<body>
	...
	*/
	public function __construct($message='',$comment='',$command='back',$action='', $headerscript='') {
		$this->message = $message;
		$this->comment = preg_replace("/[ ]+/"," ",preg_replace("/(\n|\t)+/"," ",$comment));
		$this->command = $command;
		$this->action = $action;
		$this->header .= $headerscript;
	}
		...
		switch($this->command){
			case 'back':
				if($this->message!="")
					...
					$script = $this->header."<script>history.back();</script>".$this->footer;
				break;
			case 'alert':
			...
			case 'script':
				if($this->message!='' || $this->comment!='')
					$script = $this->header."<script>alert(\"".$this->message."\\n".$this->comment."\");".$this->action."</script>".$this->footer;
				else
