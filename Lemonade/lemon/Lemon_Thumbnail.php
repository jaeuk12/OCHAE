...
*
* $th = new Lemon_Thumbnail("원본파일명(경로포함)","저장할썸네일명(확장자지정안해도됨)");
...
class Lemon_Thumbnail {
	...
	*/
	...
	function resizeImage($src,$dest,$width,$height){
		...
		$ext = strtolower($this->fileExt);								// 파일 확장자
		...
		if($ext=='jpg'){
			...
			if(!call_user_func('image'.$subFuncName,$hDResource,$dest,$rate)){
